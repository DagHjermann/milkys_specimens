...
- adapted from `150_Get_fish_individual_data_2019.Rmd` from Milkys2 project in Jupyterhub
- which again is based on '03_Read_fish_individual_data.Rmd' from Milkys2 in Jupyterhub  
...
year <- 2015
filename_save_rds <- "020_Fish_individual_data_2015.rds"
filename_save_csv <- "020_Fish_individual_data_2015.csv"
datafolder <- "data_input/Milkys_opparbeiding_fisk_2015/"

check <- grepl(as.character(year), 
               filename_save_rds, filename_save_csv, datafolder)

if (mean(check) != 1)
  stop("Check file names and folder - years must correspond!")
...
# For now, alos delete this:  
sel <- fns %in% c(
  "ny_13B_Kr_sand_lis_2016_jan_xtrainnsamling.xlsx",
  "15B_Farsund_torsk2015.xlsx")
fns <- fns[!sel]
...
dat <- read_excel(paste0(datafolder, "/", fn), skip = line_header)
...
- note: get_columns() used further down will find columns by name regardless of case,
so it will find both 'lengde (cm)' and 'Lengde (cm)'

```{r}
# Get column name for a given column number 
get_colname <- function(df, colnumber) colnames(df)[colnumber]

# Correct one column in one file:
# View(dat_list[[3]])
# dat_list[[3]] <- dat_list[[3]] %>%
#   rename(Levervekt = Lever)

for (j in seq_along(dat_list)){
  nm <- names(dat_list[[j]])
  sel <- grepl("lever", nm, ignore.case = TRUE) & grepl("prøve", nm, ignore.case = TRUE) 
  if (sum(sel) == 1)
    names(dat_list[[j]])[sel] <- "Leverprøve"
  sel <- grepl("lever", nm, ignore.case = TRUE) & grepl("vekt", nm, ignore.case = TRUE) 
  if (sum(sel) == 1)
    names(dat_list[[j]])[sel] <- "Levervekt"
  sel <- grepl("lever", nm, ignore.case = TRUE) & grepl("farge", nm, ignore.case = TRUE) 
  if (sum(sel) == 1)
    names(dat_list[[j]])[sel] <- "Leverfarge"
  sel <- grepl("gonade", nm, ignore.case = TRUE) & grepl("vekt", nm, ignore.case = TRUE) 
  if (sum(sel) == 1)
    names(dat_list[[j]])[sel] <- "Gonadevekt"
  sel1 <- grepl("kysttorsk", nm, ignore.case = TRUE) & grepl("skrei", nm, ignore.case = TRUE) 
  sel2 <- grepl("otolitt", nm, ignore.case = TRUE) & grepl("form", nm, ignore.case = TRUE) 
  sel <- sel1 | sel2
  if (sum(sel) == 1)
    names(dat_list[[j]])[sel] <- "Otolittform"
}

# Get all column names, by column number 
for (i in seq_along(names(dat_list[[1]]))){
  cat(i, "\n")
...
# Add columns that are lacking  
# View(dat_list[[4]])
dat_list[[4]]$Alder <- NA                      # HARD-CODED
dat_list[[4]]$Otolittform <- as.character(NA)  # HARD-CODED

dat_list_selected <- dat_list %>% 
  map(get_columns, colnames = c("Station", "Species", "Fisknr", "Lengde", "Vekt", "Kjønn", 
                                "Levervekt", "Leverfarge", "Gonadevekt", "Alder", "Otolittform"))
...
### Fix Station column 1
```{r}
cat("Existing values of in data:\n")
dat_list_selected %>% map(~unique(.$Station))  

for (j in seq_along(dat_list_selected)){
  dat_list_selected[[j]] <- as.data.frame(dat_list_selected[[j]])
  dat_list_selected[[j]][,1] <- dat_list_selected[[j]][,1] %>% 
    str_extract("([^[[:blank:]]]+)") %>%
    toupper()
}


cat("New values of in data:\n")
dat_list_selected %>% map(~unique(.$Station))  

```

### Fix Station column 2  

- set station names in data based on file name  
- note; HARD-CODED!
 
```{r}
cat("\n\nStations - Get file names:\n")
fns_to_read[c(8,13,14,18)]
dat_list_selected[[8]]$Station <- "28B"    # HARD-CODED!
dat_list_selected[[13]]$Station <- "43B"    # HARD-CODED!
dat_list_selected[[14]]$Station <- "45B"    # HARD-CODED!
dat_list_selected[[18]]$Station <- "80B"    # HARD-CODED!
...
### Check/fix Species (NOTE: HARD-CODED FILE NUMBERS)
...
for (j in seq_along(dat_list_selected)){
  dat_list_selected[[j]] <- dat_list_selected[[j]] %>% 
    mutate(
      Species = as.character(Species),
      Species = case_when(
        Species == "GADMO" ~ "Torsk",
        Species == "GADU.MOR" ~ "Torsk",
        grepl("torsk", Species, ignore.case = TRUE) ~ "Torsk",
        grepl("sandflyndre", Species, ignore.case = TRUE) ~ "Sandflyndre",
        Station %in% c("43B2","96B","43B","28B","80B") ~ "Torsk",
        TRUE ~ Species)
    )
}
...
# View(dat_list_selected[[4]])

dat_list_selected %>% map("Kjønn")

for (j in seq_along(dat_list_selected)){
  if (is.numeric(dat_list_selected[[j]]$Kjønn)){
    dat_list_selected[[j]] <- dat_list_selected[[j]] %>%
      mutate(
        Kjønn = case_when(
          Kjønn %in% 1 ~ "M",
    ...
    dat_list_selected[[j]]$Kjønn <- toupper(dat_list_selected[[j]]$Kjønn)
  }
...
saveRDS(dat, paste0("Data/", filename_save_rds))
readr::write_csv(dat, paste0("Data/", filename_save_csv)) 

...
- copy of `150_Get_fish_individual_data_2019.Rmd` from Milkys2 project in Jupyterhub
...
year <- 2018
filename_save_rds <- "020_Fish_individual_data_2018.rds"
filename_save_csv <- "020_Fish_individual_data_2018.rds"
datafolder <- "data_input/Milkys_opparbeiding_fisk_2018/"
...
sel <- fns %in% c("ikke_bruk_milkys data 2018 Svalbard og Tromsø - akvaplan-niva.xlsx", 
                  "MILKYS+Lange tidsserier - analyser 2018.xlsx", "oversikt_over_fisk_2018.xlsx",
                  "2018_MAL-FISK-stasjonskode_stasjonsnavn.xlsx")
fns <- fns[!sel]

# For now, alos delete this:  
# sel <- fns %in% "2018_Brosme_S2_Sørfjorden.xlsx"
# fns <- fns[!sel]
...
for (j in seq_along(dat_list_selected)){
  dat_list_selected[[j]] <- dat_list_selected[[j]] %>% 
    mutate(
      ...
      Species = case_when(
        Species == "GADMO" ~ "Torsk",
        Station %in% c("43B2","96B","43B") ~ "Torsk",
        TRUE ~ Species)
    )
}
...
write.csv(dat, paste0("Data/", filename_save_csv), fileEncoding = "UTF-8") 
